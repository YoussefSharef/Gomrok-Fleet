...
export function LoginScreen() {
  ...
  const noAccounts = !loading && !loadError && people.length === 0;
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
      ...
      >
        ...
        {loadError && (
          <div style={{ padding: '11px 13px', borderRadius: 3, background: colors.redBg, color: colors.red, fontSize: 12.5, fontWeight: 600, lineHeight: 1.5 }}>
            {t.login.serverUnreachable}
          </div>
        )}
        {noAccounts && (
          <div style={{ padding: '11px 13px', borderRadius: 6, background: colors.bg, border: `1px solid ${colors.border}`, fontSize: 12.5, color: colors.textBody, lineHeight: 1.6 }}>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>{t.login.noAccounts}</div>
            <div>Run <code>npm run seed</code> in <code>server/</code> to create the default Administrator account (PIN 1234), then reload this page.</div>
          </div>
        )}
        {!loadError && !noAccounts && (
          <>
